...
```bash
python pi/pi-config-transfer.py export            # -> ./pi-setup-export/ (in cwd)
```
...
```bash
python pi/pi-config-transfer.py import
```
...
| Command | Purpose |
| :--- | :--- |
| `python pi/pi-config-transfer.py export [dest]` | Create export folder (default `./pi-setup-export/`) |
| `python pi/pi-config-transfer.py export --include-secrets` | Also export API keys + auth.json (models.json apiKey, tavily key, login tokens) |
| `python pi/pi-config-transfer.py export --dry-run` | Preview what would be exported |
| `python pi/pi-config-transfer.py import [src]` | **Full provisioning** (7 steps above); defaults to `./pi-setup-export/` |
| `python pi/pi-config-transfer.py import [src] --no-install` | Config files only — skip pi/packages/binaries/patch |
| `python pi/pi-config-transfer.py import [src] --dry-run` | Preview what would be applied |
| `python pi/pi-config-transfer.py list [src]` | Show packages + items inside an export; defaults to `./pi-setup-export/` |
| `python pi/pi-config-transfer.py check` | Show environment info (platform, paths, tool availability) |
