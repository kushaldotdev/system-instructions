...
class PromptContractTests(unittest.TestCase):
    ...
    def test_installers_include_skill_and_audit_contracts(self) -> None:
        ...
        self.assertIn("wslpath", installer)
        self.assertIn("py.exe", installer)
        self.assertIn("wsl.exe", installer)
        self.assertIn('"--wsl-distro"', installer)
        readme = (REPOSITORY_ROOT / "README.md").read_text(encoding="utf-8")
        self.assertIn("--environment windows", readme)
        self.assertIn("--environment wsl", readme)
        self.assertIn("--environment both", readme)
        self.assertIn("target environment's own Python", readme)
