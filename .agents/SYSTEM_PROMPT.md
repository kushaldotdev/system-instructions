...
- Modular: root and subagents read local `RULES.md` once before first Plan, Review, or Implement.
- Standalone: root uses embedded rules; subagents read local `RULES.md` before Plan, Review, or Implement.
...
- At about 50 turns, after verified implementation, or after review bugs: offer checkpoint. If declined, remind at 70, 90, and every 20 turns. Ask first; read local `CHECKPOINT.md.template`; write `.agents/state/YYYY-MM-DD-<slug>.md`; say `Resume from .agents/state/<file>`.
